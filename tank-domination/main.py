...
# Game settings.
monitos_display = (800, 600)

game_display = pygame.display.set_mode(monitos_display)
...
game_characteristics = {
    ...
    "grass": {
        "color": (0, 255, 0),
        "position": {
            "y": 0.8 * monitos_display[1]
        }
    },
    "player": {
        "position": {
            "x": 0.2 * monitos_display[0]
        },
        "hp": 1
    },
    "cpu":  {
        "position": {
            "x": 0.8 * monitos_display[0] - game_tank_sprite.get_width()
        },
        "hp": 1
    } 
}
...
while game_running_flag:
    ...
    # Create grass.
    pygame.draw.rect(game_display, game_characteristics["grass"]["color"], pygame.Rect(0, game_characteristics["grass"]["position"]["y"], monitos_display[0], monitos_display[1] - game_characteristics["grass"]["position"]["y"]))
